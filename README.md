...
First you have to install the `babel-preset-react` preset, and the React packages - `react 14.x` and `react-dom 14.x`. Then you need to modify the `babelrc` file so it looks like this `"presets": [ "es2015", "stage-0", "react" ]`. And to get `JSX` working in the unit tests, you need to import the React library - `import React from 'react'.`
